@extends('layouts.employee', ['title' => 'Absen Masuk'])
...
<div class="space-y-4">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h2 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Verifikasi Kehadiran</h2>
            <p class="text-xs text-slate-500 dark:text-slate-400">Pastikan wajah terlihat jelas di area kamera</p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800/90 border border-slate-200 dark:border-slate-700/80 rounded-3xl p-4 shadow-sm dark:shadow-2xl overflow-hidden relative">
        ...
        <!-- Viewport Container (Aspect Ratio 1:1 or 4:3) -->
        <div class="relative w-full aspect-square bg-black rounded-2xl overflow-hidden flex items-center justify-center border border-slate-200 dark:border-slate-700 shadow-inner">
            ...
            <!-- 2. Captured Canvas Image (Hidden until capture) -->
            <canvas id="photo-canvas" class="hidden w-full h-full object-cover"></canvas>

            <!-- 3. Face Positioning Oval Overlay Guide -->
            <div id="face-overlay" class="absolute inset-0 pointer-events-none flex items-center justify-center">
                <div class="w-48 h-64 border-2 border-dashed border-brand-400/80 rounded-[50%] shadow-[0_0_0_9999px_rgba(0,0,0,0.4)]"></div>
            ...
            <div id="camera-error" class="hidden absolute inset-0 bg-slate-900 p-6 flex flex-col items-center justify-center text-center gap-3">
                ...
                <button type="button" onclick="startCamera()" class="mt-2 px-4 py-2 bg-brand-600 hover:bg-brand-500 text-white text-xs font-semibold rounded-xl transition-colors">
                    Coba Lagi
                </button>
            </div>
        ...
        <div class="mt-4">
            ...
            <div id="capture-controls" class="flex items-center justify-center">
                <button 
                    ...
                    disabled
                    class="w-16 h-16 rounded-full bg-gradient-to-tr from-brand-600 to-indigo-500 border-4 border-slate-200 dark:border-slate-800 shadow-xl shadow-brand-500/40 flex items-center justify-center text-white active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all cursor-pointer"
                >
                    <i data-lucide="camera" class="w-7 h-7"></i>
                </button>
            </div>

            <!-- State: Captured (Confirm / Retake) -->
            <div id="confirm-controls" class="hidden grid grid-cols-2 gap-3">
                ...
                <button 
                    ...
                    class="py-3 px-4 bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-bold rounded-2xl text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-500/30 transition-all cursor-pointer"
                >
    ...
    <!-- Branch Target Info -->
    <div class="bg-white dark:bg-slate-800/60 border border-slate-200 dark:border-slate-700/40 rounded-2xl p-3 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
        <div class="flex items-center gap-2">
            <i data-lucide="map-pin" class="w-4 h-4 text-brand-500 dark:text-brand-400 shrink-0"></i>
            <span>Cabang: <strong class="text-slate-800 dark:text-slate-200">{{ $branch->name }}</strong></span>
        </div>
        <span>Maks Radius: {{ $branch->radius_meter }}m</span>
    </div>
    ...
    <div id="modal-success" class="hidden fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
        <div class="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-3xl p-6 max-w-sm w-full text-center space-y-4 shadow-2xl">
            <div class="w-16 h-16 rounded-full bg-emerald-500/20 text-emerald-500 dark:text-emerald-400 flex items-center justify-center mx-auto">
                <i data-lucide="check-circle-2" class="w-10 h-10"></i>
            </div>
            <div>
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Absensi Masuk Berhasil!</h3>
                <p id="success-desc" class="text-xs text-slate-500 dark:text-slate-400 mt-1">Kehadiran Anda telah dicatat oleh sistem.</p>
            </div>
            ...
            <div class="bg-slate-50 dark:bg-slate-900/80 rounded-2xl p-3 text-xs space-y-1.5 text-left border border-slate-200 dark:border-slate-700/60">
                <div class="flex justify-between">
                    <span class="text-slate-500 dark:text-slate-400">Jam Masuk:</span>
                    <strong id="res-time" class="text-slate-900 dark:text-white">--:--:--</strong>
                </div>
                <div class="flex justify-between">
                    <span class="text-slate-500 dark:text-slate-400">Status Kehadiran:</span>
                    <strong id="res-status" class="text-emerald-600 dark:text-emerald-400">Tepat Waktu</strong>
                </div>
                <div class="flex justify-between">
...
<script>
    ...
    let gpsSamples = [];
    const GPS_SAMPLE_TARGET = 3;    // jumlah sample yang ingin dikumpulkan
    const GPS_SAMPLE_INTERVAL = 2000; // interval antar sample (ms)
    ...
    async function startCamera() {
        ...
        try {
            ...
            if (err.name === 'NotAllowedError' || err.name === 'PermissionDeniedError') {
                errorMsgEl.textContent = 'Izin akses kamera ditolak. Silakan berikan izin kamera pada browser Anda untuk melanjutkan absensi.';
            } else {
                errorMsgEl.textContent = 'Kamera tidak dapat diakses atau sedang digunakan oleh aplikasi lain: ' + err.message;
            }
        }
    }

    // 2. Initialize Geolocation — kumpulkan multiple samples
    function getGPSLocation() {
        ...
        // Ambil sample pertama segera
        collectGPSSample();

        // Lanjutkan mengumpulkan sample setiap interval
    ...
    function collectGPSSample() {
        navigator.geolocation.getCurrentPosition(
            (pos) => {
                ...
                // Gunakan sample terbaru sebagai koordinat utama
                currentCoords = {
            ...
            (err) => {
                ...
                // Hanya tampilkan error jika sama sekali belum ada sample
    ...
    function takeSnapshot() {
        ...
        // Mirror snapshot because user-facing camera is mirrored
    ...
    async function submitAttendance() {
        ...
        try {
            ...
            if (response.ok && result.success) {
                document.getElementById('res-time').textContent = result.data.check_in_at + ' WIB';
                document.getElementById('res-status').textContent = result.data.status;
                document.getElementById('res-distance').textContent = result.data.distance + ' meter';
                document.getElementById('modal-success').classList.remove('hidden');
            } else {
                alert(result.message || 'Gagal melakukan absensi masuk.');
